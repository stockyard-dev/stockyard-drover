package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-drover/internal/store")
...
func(s *Server)routes(){
    ...
    s.mux.HandleFunc("PATCH /api/jobs/{id}",s.handleUpdateJob)
    s.mux.HandleFunc("POST /api/jobs/{id}/retry",s.handleRetryJob)
    s.mux.HandleFunc("DELETE /api/jobs/{id}",s.handleDeleteJob)
    s.mux.HandleFunc("GET /",s.handleUI)
}
func(s *Server)handleHealth(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-drover"})}
