...
.stats{display:grid;grid-template-columns:repeat(5,1fr);gap:.5rem;margin-bottom:1.2rem}
...
.queues{display:flex;gap:.3rem;margin-bottom:1rem;flex-wrap:wrap}
.q-btn{font-size:.6rem;padding:.25rem .6rem;border:1px solid var(--bg3);background:var(--bg);color:var(--cm);cursor:pointer}.q-btn:hover{border-color:var(--leather)}.q-btn.active{border-color:var(--rust);color:var(--rust)}
.job{background:var(--bg2);border:1px solid var(--bg3);padding:.6rem .8rem;margin-bottom:.3rem;font-size:.72rem}
...
.badge-pending{background:#d4843a22;color:var(--orange);border:1px solid #d4843a44}
.badge-running{background:#4a7ec922;color:#4a7ec9;border:1px solid #4a7ec944}
...
.job-payload{font-size:.65rem;color:var(--cm);margin-top:.2rem;max-height:40px;overflow:hidden}
.job-meta{font-size:.55rem;color:var(--cm);margin-top:.2rem}
...
<div class="hdr"><h1>DROVER</h1><div style="display:flex;gap:.4rem"><button class="btn btn-p" onclick="openJob()">+ Enqueue Job</button><button class="btn" onclick="openQueue()">+ Queue</button></div></div>
<div class="main">
...
<script>
const A='/api';let queues=[],jobs=[],curQ='',stats={};
async function load(){const[q,s]=await Promise.all([fetch(A+'/queues').then(r=>r.json()),fetch(A+'/stats').then(r=>r.json())]);
queues=q.queues||[];stats=s;
const p=stats.pending||0,r=stats.running||0,c=stats.completed||0,f=stats.failed||0;
document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+stats.total+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v" style="color:var(--orange)">'+p+'</div><div class="st-l">Pending</div></div><div class="st"><div class="st-v" style="color:#4a7ec9">'+r+'</div><div class="st-l">Running</div></div><div class="st"><div class="st-v" style="color:var(--green)">'+c+'</div><div class="st-l">Done</div></div><div class="st"><div class="st-v" style="color:var(--red)">'+f+'</div><div class="st-l">Failed</div></div>';
...
function setQ(q){curQ=q;loadJobs();}
async function loadJobs(){let url=A+'/jobs?limit=100';if(curQ)url+='&queue='+curQ;
const r=await fetch(url).then(r=>r.json());jobs=r.jobs||[];renderJobs();}
...
h+='<div class="job"><div class="job-top"><span>'+esc(j.id.slice(-8))+'</span><div style="display:flex;gap:.3rem;align-items:center"><span class="badge badge-'+j.status+'">'+j.status+'</span>';
if(j.status==='failed')h+='<button class="btn" onclick="retry(\''+j.id+'\')" style="font-size:.5rem">Retry</button>';
h+='</div></div>';
if(j.payload)h+='<div class="job-payload">'+esc(j.payload.substring(0,120))+'</div>';
h+='<div class="job-meta">Attempt '+j.attempts+'/'+j.max_attempts;
if(j.error)h+=' · Error: '+esc(j.error.substring(0,60));
h+=' · Created '+ft(j.created_at);
if(j.finished_at)h+=' · Finished '+ft(j.finished_at);
h+='</div></div>';});
...
document.getElementById('mdl').innerHTML='<h2>Enqueue Job</h2><div class="fr"><label>Queue</label><select id="f-q">'+opts+'</select></div><div class="fr"><label>Payload (JSON)</label><textarea id="f-p" rows="4" placeholder=\'{"action":"send_email","to":"user@example.com"}\'></textarea></div><div class="fr"><label>Priority (0=normal, higher=sooner)</label><input id="f-pr" type="number" value="0"></div><div class="acts"><button class="btn" onclick="cm()">Cancel</button><button class="btn btn-p" onclick="subJ()">Enqueue</button></div>';document.getElementById('mbg').classList.add('open');}
